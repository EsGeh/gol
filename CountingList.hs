...
module CountingList(CountingList,empty,get,(CountingList.!!),cons,clLength,toList,fromList,module Card.Card,module Card.Unary) where
import Card.Card
import Card.Unary
...
get i cl@(CL list)
	| (i >= 0) && (i< length list) = Just $ cl CountingList.!! i
	| otherwise = Nothing
